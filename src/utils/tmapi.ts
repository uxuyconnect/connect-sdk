...
const logDebug = debug("uxuy:tmapi:debug")
const logError = debug("uxuy:tmapi:error")
...
class UxuyConnectUIError extends Error {
    constructor(message: string) {
        super(message);
        this.name = 'UxuyConnectUIError';
    }
}
...
export function sendOpenTelegramLink(link: string, fallback?: () => void): void {
    ...
    if (url.protocol !== 'http:' && url.protocol !== 'https:') {
        ...
        throw new UxuyConnectUIError(`Url protocol is not supported: ${url}`);
    }
    if (url.hostname !== 't.me') {
        if (fallback) {
            return fallback();
        }
        throw new UxuyConnectUIError(`Url host is not supported: ${url}`);
    }
...
function postEvent(eventType: string, eventData: object): void {
    try {
        const window = getWindow();
        if (!window) {
            throw new UxuyConnectUIError(`Can't post event to parent window: window is not defined`);
        }
        ...
        if (window.TelegramWebviewProxy !== undefined) {
            ...
            throw new UxuyConnectUIError(`Can't post event to TMA`);
        }
    } catch (e) {
